...
class AddressBindFailedState(BaseGameState):
    ...
    def get_enum_value(self) -> GameState:
        return GameState.MENU_ERROR_ADDRESS_BIND
    ...
    def render(self) -> None:
        game_over_text = GameFont.heading1_font.render("ERROR", True, Color.BLACK)
        game_over_text_rect = game_over_text.get_rect(center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 - 50))
        self.screen.blit(game_over_text, game_over_text_rect)

        final_score_text = GameFont.text_font.render(f"Could not bind port 12345. Already in use!", True, Color.BLACK)
        final_score_text_rect = final_score_text.get_rect(center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 10))
        self.screen.blit(final_score_text, final_score_text_rect)

        final_score_text = GameFont.text_font.render(f"Please restart the game!", True, Color.BLACK)
        final_score_text_rect = final_score_text.get_rect(center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 + 40))
        self.screen.blit(final_score_text, final_score_text_rect)
